...
// Treats any src value starting with 'assets/' (or absolute URL) as a real image.
// Anything else (undefined, 'placeholder', null) falls back to the generated SVG.
function realSrc(s) {
  return s && (s.startsWith('assets/') || s.startsWith('http') || s.startsWith('data:')) ? s : null;
}
function imgSrc(img, w, h, label) {
  return realSrc(img?.src) || window.placeholderImage(img?.tone, w || 2000, h || 1300, label || '');
}
function coverSrc(p, w, h) {
  return realSrc(p?.cover) || imgSrc(p?.images?.[0], w || 2400, h || 1500, '');
}
function indexCoverSrc(p, w, h) {
  return realSrc(p?.indexCover) || realSrc(p?.cover) || imgSrc(p?.images?.[0], w || 800, h || 1000, '');
}
// Strip trailing scale annotations like "1:200", "1：500" from a drawing label.
function stripScale(label) {
  if (label == null) return '';
  if (typeof label === 'object') {
    return { en: stripScale(label.en || ''), zh: stripScale(label.zh || '') };
  }
...
function PortraitIcon({ size = 28, className = '', src = 'assets/portrait-white.png' }) {
  return (
    <img
      src={src}
      alt=""
      className={`c-portrait ${className}`}
      style={{ ...{ width: size, height: size }, objectFit: "contain", height: "36px", width: "36px" }} />);
  ...
  return (
    <aside
      ...
      <button className="c-rail-mark" onClick={() => setRoute({ page: 'home' })}>
        <span className="c-rail-mark-mini"><PortraitIcon size={32} src="assets/portrait-sketch.png" className="c-portrait-sketch" /></span>
...
function CHomeStage({ onScrollToCinema }) {
  ...
  React.useEffect(() => {
    ...
    const tick = () => {
      ...
      // Fade: hold at 1 during hero (0–0.5), fade 1→0.18 during about (0.5–0.85), hold (0.85–1)
      let fade;
      if (p < 0.5) fade = 1;else
      if (p > 0.85) fade = 0.18;else
      fade = 1 - (p - 0.5) / 0.35 * 0.82;
      ...
      <div className="c-home-narrative">
        ...
        <div className="c-hero-screen" data-screen-label="C · Hero">
          <div className="c-hero-top">
            <div className="c-tag" style={{ color: "rgb(255, 255, 255)" }}>{lang === 'en' ? 'Selected works · 2015—2026' : '作品选 · 2015—2026'}</div>
          ...
          <div className="c-hero-bottom">
            ...
            <button className="c-hero-enter" onClick={scrollToAbout}>
              <span>{lang === 'en' ? 'About me' : '关于我'}</span>
          ...
          <div className="c-hero-scrollhint" aria-hidden="true">
            ...
            <span className="c-hero-scrollhint-text">{lang === 'en' ? 'scroll to play' : '滚动继续'}</span>
          </div>
        </div>
...
function CCinemaItem({ project: p, idx, setRoute }) {
  ...
  return (
    <article
      ...
      <div className="c-frame-bottom" style={{ backgroundColor: "rgba(0, 0, 0, 0.863)", opacity: "0.5", width: "450px" }}>
  ...
  return (
    <article className="c-project" data-screen-label="C · Project">
      ...
      <div className="c-cover">
        <img src={coverSrc(p, 2400, 1500)} alt="" />
      ...
      <div className="c-description">
          {ctx(p.description, lang).split('\n\n').map((para, i) =>
        <p key={i} className="c-description-p">{para}</p>
        )}
        </div>
      }
      ...
      <div className="c-plates">
        {p.images.map((img, i) =>
        <CPlate key={i} idx={i + 1} src={imgSrc(img, 2000, 1200, '')} caption={img.label} />
        )}
      </div>
      ...
      <div className="c-drawings-block">
          ...
          d.size === 'large' ? true :
          d.size === 'small' ? false :
          d.kind === 'plan' || d.kind === 'perspective-section';
          const large = p.drawings.filter(isLarge);
          const small = p.drawings.filter((d) => !isLarge(d));
          return (
            <>
                {large.length > 0 &&
              <div className="c-drawings-large">
                  {large.map((d, i) =>
                <figure key={'l' + i} className={`c-drawing c-drawing-large c-drawing-${d.kind}`}>
                    <img
                    src={realSrc(d.src) || window.placeholderImage('paper-dark', 1800, 1200, ctx(stripScale(d.label), lang))}
                    alt="" loading="lazy" />
                    <figcaption>{ctx(stripScale(d.label), lang)}</figcaption>
                  </figure>
                )}
                </div>
              }
              ...
              <div className="c-drawings">
                  {small.map((d, i) =>
                <figure key={'s' + i} className={`c-drawing c-drawing-${d.kind}`}>
                    <img
                    src={realSrc(d.src) || window.placeholderImage('paper-dark', 1400, 1000, ctx(stripScale(d.label), lang))}
                    alt="" loading="lazy" />
                    <figcaption>{ctx(stripScale(d.label), lang)}</figcaption>
                  </figure>
                )}
      ...
      <button
        ...
        <img className="c-next-img" src={coverSrc(next, 800, 600)} alt="" />
      </button>
    </article>);
    ...
    <section className="c-doc c-contact" data-screen-label="C · Contact">
      ...
      <div className="c-contact-rows">
        ...
        <div className="c-contact-row c-contact-row-static">
          ...
          <span className="c-contact-val">Los Angeles, California · <span className="c-mono-cell" style={{ opacity: "0.5", fontSize: "12px" }}>34.05°N 118.24°W</span></span>
        </div>
      </div>
    </section>);
...
function CBackToTop() {
  ...
  return (
    <button
      ...
      <PortraitIcon size={42} src="assets/portrait-sketch.png" className="c-portrait-sketch" />
    </button>);
}
...
Object.assign(window, { C_lang, ctx, fmtNum, fmtArea, fmtCoords, projectYear, realSrc, imgSrc, coverSrc, indexCoverSrc, stripScale, TrackChip, RoleChip, CoordChip, Yr, CSite });